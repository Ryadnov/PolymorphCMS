...
class FileSystem
{
	private static $_dirFileInfoCache = array(); 
		...
		if ($fp = @opendir($sourceDir)) {
			...
			while (FALSE !== ($file = readdir($fp))) {
				...
				if (($directoryDepth < 1 OR $newDepth > 0) && @is_dir($sourceDir.$file)) {
					$filedata[] = array(
						'text' => $file,
						'children' => self::directoryMap($sourceDir.$file.DIRECTORY_SEPARATOR, $newDepth, $hidden)	
					); 
				} else {
					$filedata[] = array('text' => $file);
				}
			}
 ...
 * @return	array
 */
	public static function getFileNames($sourceDir, $includePath = FALSE, $_recursion = FALSE)
	{
		...
		if ($fp = @opendir($sourceDir)) {
			...
			while (FALSE !== ($file = readdir($fp))) {
				if (@is_dir($sourceDir.$file) && strncmp($file, '.', 1) !== 0) {
					self::getFileNames($sourceDir.$file.DIRECTORY_SEPARATOR, $includePath, TRUE);
 ...
 * @return	array
 */
	public static function dirFileInfo($sourceDir, $topLevelOnly = TRUE, $_recursion = FALSE)
	{
		$hash = $sourceDir.$topLevelOnly.$_recursion;
		if (isset(self::$_dirFileInfoCache[$hash])) 
			return self::$_dirFileInfoCache[$hash];
		...
		if ($fp = @opendir($sourceDir)) {
			...
			while (FALSE !== ($file = readdir($fp))) {
				if (@is_dir($sourceDir.$file) AND strncmp($file, '.', 1) !== 0 AND $topLevelOnly === FALSE) {
					...
					$_filedata[$file] = self::getInfo($sourceDir.$file);
					$_filedata[$file]['relative_path'] = $relativePath;
				}
			}

			return self::$_dirFileInfoCache[$hash] = $_filedata;
		} else {
			return array();
		}
	}
...
* @return	array
*/
	public static function getInfo($file, $returnedValues = array('name', 'ext', 'server_path', 'size', 'date', 'baseName'))
	{
		...
		foreach ($returnedValues as $key) {
			switch ($key) {
				case 'name':
					$fileinfo['name'] = pathinfo($file, PATHINFO_FILENAME);
					break;
				case 'server_path':
 ...
 * @return	mixed
 */
	public static function mimeByExtension($file)
	{
