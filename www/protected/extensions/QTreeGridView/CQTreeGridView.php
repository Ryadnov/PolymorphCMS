...
class CQTreeGridView extends CGridView
{
    ...
    /**
     * @var string the base script URL for all treeTable views resources (e.g. javascript, CSS file, images).
    ...
    public function renderTableRow($row)
    {
        ...
        {
            echo '<tr id="'.$model->getPrimaryKey().'" class="'.($ir ? 'root' : '').' '.$parentClass.$this->evaluateExpression($this->rowCssClassExpression,array('row'=>$row,'data'=>$model)).'">';
        }
        else if(is_array($this->rowCssClass) && ($n=count($this->rowCssClass))>0)
            echo '<tr id="'.$model->getPrimaryKey().'" class="'.($ir ? 'root' : '').' '.$parentClass.$this->rowCssClass[$row%$n].'">';
        else
            echo '<tr id="'.$model->getPrimaryKey().'" class="'.($ir ? 'root' : '').' '.$parentClass.'">';
