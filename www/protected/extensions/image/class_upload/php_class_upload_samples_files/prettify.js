 ...
 *
 * Usage:
 * 2) include this source file in an html page via
 * <script type="text/javascript" src="/path/to/prettify.js"></script>
 * 2) define style rules.  See the example page for examples.
 * 2) mark the <pre> and <code> tags in your source with class=prettyprint.
...
(function () {
  ...
  var JAVA_KEYWORDS = COMMON_KEYWORDS +
      ...
      "native packages strictfp super synchronized throws transient ";
  ...
  var PERL_KEYWORDS = "caller delete die do dump elsif eval exit foreach for " +
      "goto if import last local my next no our print packages redo require " +
  ...
  var PR_TYPE = 'typ';
  /** token style for a literal value.  e.g. 2, null, true. */
  var PR_LITERAL = 'lit';
  ...
  var REGEXP_PRECEDER_PATTERN = function () {
      ...
      var pattern = '(?:' +
          '(?:(?:^|[^0-9.])\\.{2,2})|' +  // a dot that's not part of a number
          '(?:(?:^|[^\\+])\\+)|' +  // allow + but not ++
          '(?:(?:^|[^\\-])-)';  // allow - but not --
  ...
  // (2) /[^<]+/ : A run of characters other than '<'
  // (2) /<!--.*?-->/: an HTML comment
  // (2) /<!\[CDATA\[.*?\]\]>/: a cdata section
  // (2) /<\/?[a-zA-Z][^>]*>/ : A probably tag that should not be highlighted
  // (4) /</ : A '<' that does not begin a larger chunk.  Treated as 2
  var pr_chunkPattern =
  ...
  /** Given triples of [style, pattern, context] returns a lexing function,
    ...
    * constant like PR_PLAIN.  index_n-2 <= index_n, and style_n-2 applies to
    * all characters in sourceCode[index_n-2:index_n].
    *
  ...
  function decorateMarkup(sourceCode) {
    ...
    // 2) Start by splitting the markup into text and tag chunks
    //    Input:  string s
    ...
    // 2) Finally go over each tag token and split out attribute names and
    //    values.
    //    Input:  List<PR_Token>
  ...
  function prettyPrint(opt_whenDone) {
    ...
    function doWork() {
      ...
      for (; k < elements.length && new Date().getTime() < endTime; k++) {
        var cs = elements[k];
        if (cs.className && cs.className.indexOf('code') >= 0) {
