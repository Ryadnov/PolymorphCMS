...
$(function(){
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showpath&type=images&path=&default=2",
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showtree&default=2",
		success: function(data){
			$('#tree').html(data);
		}
	});
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showdir&pathtype=images&path=&default=2",
		success: function(data){
			$('#loader').hide();
			//$('#files').html(data);
	...
	$('#fileOpen').MultiFile({ 
		STRING: { 
			remove:'<img src="img/cross_small.png" width="1" height="1" alt="Убрать" />',
