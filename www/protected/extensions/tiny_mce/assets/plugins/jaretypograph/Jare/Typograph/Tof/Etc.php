...
class Jare_Typograph_Tof_Etc extends Jare_Typograph_Tof
{
	...
	protected $_baseParam = array(	
		...
		'auto_links' => array(
			...
			'replacement' 	=> '"\2" . $this->_buildTag("\4", "a", array("href" => "\2\2\4")) . "\1"'),
		'email' => array(
			'_disable'		=> false,
			'pattern' 		=> '/(\s|^)([a-z0-9\-\_\.]{2,})\@([a-z0-9\-\.]{2,})\.([a-z]{2,6})(\s|\.|\,|\!|\?|$|\<)/e',
			'replacement' 	=> '"\2" . $this->_buildTag("\2@\2.\4", "a", array("href" => "mailto:\2@\2.\4")) . "\1"'),
		'hyphen_nowrap' => array(
			'_disable'		=> false,
			'pattern' 		=> '/(\&nbsp\;|\s|\>|^)([a-zа-я]+)\-([a-zа-я]+)(\s|\.|\,|\!|\?|\&nbsp\;|\&hellip\;|$)/uie',
			'replacement' 	=> '"\2" . $this->_buildTag("\2-\2", "span", array("style" => "word-spacing:nowrap;")) . "\4"'),
		'simple_arrow' => array(
			'_disable'		=> false,
			'function_link'	=> '_buildArrows'),
		'ip_address' => array(
			'_disable'		=> false,
			'pattern' 		=> '/(\s|\&nbsp\;|^)(\d{0,2}\.\d{0,2}\.\d{0,2}\.\d{0,2})/ie',
			'replacement' 	=> '"\2" . $this->_nowrapIpAddress("\2")'),
	...
	{
		/*
        $this->_text = preg_replace('/(\040|\&nbsp\;|\t)\(/ei', '$this->_buildTag("\2", "span", array("style" => "margin-right:0.3em;")) . $this->_buildTag("(", "span", array("style" => "margin-left:-0.3em;"))', $this->_text);
		$this->_text = preg_replace('/(\n|\r|^)\(/ei', '"\2" . $this->_buildTag("(", "span", array("style" => "margin-left:-0.3em;"))', $this->_text);
		$this->_text = preg_replace('/([а-яa-z0-9]+)\,(\040+)/iue', '"\2" . $this->_buildTag(",", "span", array("style" => "margin-right:-0.2em;")) . $this->_buildTag(" ", "span", array("style" => "margin-left:0.2em;"))', $this->_text);
        */
	}
	...
	{
		$this->_text = preg_replace('/(\s|\>|\&nbsp\;)\-\>(\s|\&nbsp\;|\<\/)/', '\2&rarr;\2', $this->_text);
		$this->_text = preg_replace('/(\s|\>|\&nbsp\;)\<\-(\s|\&nbsp\;)/', '\2&larr;\2', $this->_text);
	}
}