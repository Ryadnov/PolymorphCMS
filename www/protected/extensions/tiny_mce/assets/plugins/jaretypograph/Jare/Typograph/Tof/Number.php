...
class Jare_Typograph_Tof_Number extends Jare_Typograph_Tof
{
	...
	protected $_baseParam = array(
		...
		'numeric_sub' => array(
			'_disable'		=> false,
			'pattern' 		=> '/([a-zа-я0-9])\_([\d]{2,2})([^а-яa-z0-9]|$)/ieu',
			'replacement' 	=> '"\2" . $this->_buildTag($this->_buildTag("\2","small"),"sub") . "\2"'),
		'numeric_sup' => array(
			'_disable'		=> false,
			'pattern' 		=> '/([a-zа-я0-9])\^([\d]{2,2})([^а-яa-z0-9]|$)/ieu',
			'replacement' 	=> '"\2" . $this->_buildTag($this->_buildTag("\2","small"),"sup") . "\2"'),
	...
	/**
	 * Преобразование простых дробей (2/2, 2/4 и 2/4) в HTML-коды
	 *
	 * @return 	void
	 */
	protected function _buildSimpleFraction()
	{
		$this->_text = preg_replace('/(\D)2\/(2|4)(\D)/', '\2&frac1\2\2', $this->_text);
		$this->_text = preg_replace('/(\D)2\/4(\D)/', '\2&frac34\2', $this->_text);
	}
	...
	protected function _buildTimesx()
	{
		...
		while(preg_match($regExpMask, $this->_text)) {
			$this->_text = preg_replace($regExpMask, '\2\2&times;\6', $this->_text);
		}
	}
