...
class Jare_Typograph_Tof_Quote extends Jare_Typograph_Tof
{
	...
	protected function _buildOpenQuote()
	{
		...
		while(preg_match($regExpMask, $this->_text)) {
			$this->_text = preg_replace($regExpMask . 'e', '"\2" . self::QUOTE_FIRS_OPEN . "\2"', $this->_text);
		}
	}
	...
	protected function _buildCloseQuote()
	{
		...
		while(preg_match($regExpMask, $this->_text)) {
			$this->_text = preg_replace($regExpMask . 'e', '"\2" . self::QUOTE_FIRS_CLOSE . "\2"', $this->_text);
		}
	}
	...
	{
		/*
        $this->_text = preg_replace('/([a-zа-я\-]{2,})(\040|\&nbsp\;|\t)(\&laquo\;)/uie', '"\2" . $this->_buildTag("\2", "span",array("style" => "margin-right:0.44em;")) . $this->_buildTag("\2", "span", array("style" => "margin-left:-0.44em;"))', $this->_text);
		$this->_text = preg_replace('/(\n|\r|^)(\&laquo\;)/ei', '"\2" . $this->_buildTag("\2", "span", array("style" => "margin-left:-0.44em;"))', $this->_text);
        */
	}
}