...
class Jare_Typograph_Tool
{
	...
	protected static $_charsTable = array(
		...
		'>=' 	=> array('html' => array('&ge;', '&#8805;'),
						 'utf8' => array(0x2265)),
		'2/2' 	=> array('html' => array('&frac12;', '&#189;'),
						 'utf8' => array(0x00BD)),
		'2/4' 	=> array('html' => array('&frac14;', '&#188;'),
					     'utf8' => array(0x00BC)),
		'2/4' 	=> array('html' => array('&frac34;', '&#190;'),
						 'utf8' => array(0x00BE)),
    ...
    public static function safeTagChars($text, $safe)
    {
    	...
    	if (true === $safe) {
        	$text = preg_replace('/(\<\/?)(.+?)(\>)/se', '"\2" .  ( substr(trim("\2"), 0, 2) === "a" ? "%%___"  : ""  ) . self::_encrypteContent(trim("\2"))  . "\2"', $text);
        } else {
        	$text = preg_replace('/(\<\/?)(.+?)(\>)/se', '"\2" .  ( substr(trim("\2"), 0, 2) === "%%___" ? self::_decrypteContent(substr(trim("\2"), 4)) : self::_decrypteContent(trim("\2")) ) . "\2"', $text);
        }
    	...
    	if (count(self::$_customBlocks)) {
    		$safeType = true === $safe ? "self::_encrypteContent('\\2')" : "stripslashes(self::_decrypteContent('\\2'))";
       		...
       		foreach (self::$_customBlocks as $block) {
        		$text = preg_replace("/({$block[0]})(.+?)({$block[1]})/se",   "'\\2' . $safeType . '\\2'"  , $text);
        	}
    	}
