...
(function() {
	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
					...
					each(dom.select('IMG', o.node), function(n) {
						...
						switch (n.className) {
							...
							case 'mceItemShockWave':
								...
								cb = 'http://download.macromedia.com/pub/shockwave/cabs/director/sw.cab#version=8,1,2,0';
								mt = 'application/x-director';
								break;

							case 'mceItemWindowsMedia':
								ci = ed.getParam('media_wmp6_compatible') ? '05589fa1-c356-11ce-bf01-00aa0055595a' : '6bf52a52-394a-11d3-b153-00c04f79faa6';
								cb = 'http://activex.microsoft.com/activex/controls/mplayer/en/nsmp2inf.cab#Version=1,2,52,701';
		...
		_objectsToSpans : function(ed, o) {
			...
			h = h.replace(/<object([^>]*)>/gi, '<span class="mceItemObject" $2>');
			h = h.replace(/<embed([^>]*)\/?>/gi, '<span class="mceItemEmbed" $2></span>');
			h = h.replace(/<embed([^>]*)>/gi, '<span class="mceItemEmbed" $2>');
