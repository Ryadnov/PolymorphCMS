	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		init : function(ed, url) {
			...
			function grabContent(e) {
				...
				n = dom.add(body, 'div', {id : '_mcePaste', 'class' : 'mcePaste'}, '\uFEFF<br _mce_bogus="2">');
		...
		_preProcess : function(pl, o) {
			...
			if (/class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(h) || o.wordContent) {
				...
				if (getParam(ed, "paste_convert_headers_to_strong")) {
					h = h.replace(/<p [^>]*class="?MsoHeading"?[^>]*>(.*?)<\/p>/gi, "<p><strong>$2</strong></p>");
				}
		...
		_convertLists : function(pl, o) {
			...
			each(dom.select('p', o.node), function(p) {
				...
				// Detect ordered lists 2., a. or ixv.
		...
		_insertBlockContent : function(ed, dom, content) {
			...
			// Is element within viewport if not then scroll it into views
