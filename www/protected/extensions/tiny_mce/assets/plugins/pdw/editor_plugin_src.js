	...
	tinymce.create('tinymce.plugins.pdw', {
		...
		init : function(ed, url) {
			...
			ed.addCommand('mcePDWToggleToolbars', function() {
				...
				for(j = 0; j < tbIds.length; j++){
					...
					if (DOM.isHidden(id)) {
						...
						DOM.show(id);
						t._resizeIframe(ed, tbIds[j], -toolbarHeight);
			...
			ed.onPostRender.add(function(){
				// If the setting pdw_toggle_on is set to 2 then hide toolbars and set button active
				if (ed.settings.pdw_toggle_on == 1) {
					...
					for(i = 0; i < toolbars.length; i++){
						...
						cm.setActive('pdw_toggle', 1);
						DOM.hide(id);
						t._resizeIframe(ed, tbId, toolbarHeight);
					}
				}
			});
		},
