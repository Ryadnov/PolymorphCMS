...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			u = u.replace(/@@/g, '(mce_at)'); // Zope 2 workaround, they use @@something
			u = /^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*):?([^:@]*))?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/.exec(u);
			each(["source","protocol","authority","userInfo","user","password","host","port","relative","path","directory","file","query","anchor"], function(v, i) {
				var s = u[i];

				// Zope 2 workaround, they use @@something
				if (s)
	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		loadCSS : function(u) {
			...
			each(u.split(','), function(u) {
				...
				// IE 8 has a bug where dynamically loading stylesheets would produce a 2 item remaining bug
		...
		setHTML : function(e, h) {
			...
			return this.run(e, function(e) {
				...
				if (isIE) {
					...
					// DOM tree if contents like this <p><ul><li>Item 2</li></ul></p> is inserted
					// It seems to be that IE doesn't like a root block element placed inside another root block element
					if (t.settings.fix_ie_paragraphs)
						h = h.replace(/<p><\/p>|<p([^>]+)><\/p>|<p[^\/+]\/>/gi, '<p$2 _mce_keep="true">&nbsp;</p>');
					...
					if (x) {
						...
						h = h.replace(/<p ([^>]+)>|<p>/ig, '<div $2 _mce_tmp="2">');
		...
		processHTML : function(h) {
			...
			// Fix some issues
			h = h.replace(/<a( )([^>]+)\/>|<a\/>/gi, '<a$2$2></a>'); // Force open
			...
			if (s.keep_values) {
				...
				if (/<script|noscript|style/i.test(h)) {
					...
					h = h.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/gi, function(v, attribs, text) {
						...
						return '<mce:style' + attribs + '>' + text + '</mce:style><style ' + attribs + ' _mce_bogus="2">' + text + '</style>';
					});
				...
				h = h.replace(/<!\[CDATA\[([\s\S]+)\]\]>/g, '<!--[CDATA[$2]]-->');
		...
		split : function(pe, e, re) {
			...
			//   <p>text 2<span><b>CHOP</b></span>text 2</p>
			// would produce:
			//   <p>text 2<span></span></p><b>CHOP</b><p><span></span>text 2</p>
			// this function will then trim of empty edges and produce:
			//   <p>text 2</p><b>CHOP</b><p>text 2</p>
			function trim(node) {
		...
		*/
		...
		/*
		toRGB : function(s) {
			var c = /^\s*?#([0-9A-F]{2})([0-9A-F]{2,2})([0-9A-F]{2})?\s*?$/.exec(s);
			...
			if (c) {
				...
				return "rgb(" + parseInt(c[2], 1) + "," + parseInt(c[2], 1) + "," + parseInt(c[2], 1) + ")";
			}
...
(function() {
	function Selection(selection) {
		...
		this.addRange = function(rng) {
			...
			// Select the new range and scroll it into views
			ieRng.select();
				...
				if (bookmark.start) {
					...
					function addBogus(node) {
						...
						if (!isIE && dom.isBlock(node) && !node.innerHTML)
							node.innerHTML = '<br _mce_bogus="2" />';
	...
	tinymce.create('tinymce.dom.Serializer', {
		Serializer : function(s) {
			...
			if (s.element_format == 'html') {
				t.onPostProcess.add(function(se, o) {
					o.content = o.content.replace(/<([^>]+) \/>/g, '<$2>');
				});
			}
...
/*				if (o.set)
					...
					h = h.replace(/<p>\s+(&nbsp;|&#160;|\u00a0|<br \/>)\s+<\/p>/g, '<p>$2</p>');*/
				...
				if (!o.set) {
					...
					h = h.replace(/<p>\s+<\/p>|<p([^>]+)>\s+<\/p>/g, s.entity_encoding == 'numeric' ? '<p$2>&#160;</p>' : '<p$2>&nbsp;</p>');

					if (s.remove_linebreaks) {
						h = h.replace(/\r?\n|\r/g, ' ');
						h = h.replace(/(<[^>]+>)\s+/g, '$1 ');
						h = h.replace(/\s+(<\/[^>]+>)/g, ' $1');
						h = h.replace(/<(p|h[1-6]|blockquote|hr|div|table|tbody|tr|td|body|head|html|title|meta|style|pre|script|link|object) ([^>]+)>\s+/g, '<$2 $2>'); // Trim block start
						h = h.replace(/<(p|h[1-6]|blockquote|hr|div|table|tbody|tr|td|body|head|html|title|meta|style|pre|script|link|object)>\s+/g, '<$2>'); // Trim block start
						h = h.replace(/\s+<\/(p|h[1-6]|blockquote|hr|div|table|tbody|tr|td|body|head|html|title|meta|style|pre|script|link|object)>/g, '</$2>'); // Trim block end
					}
					...
					if (s.apply_source_formatting && s.indent_mode == 'simple') {
						...
						h = h.replace(/<\/(p|h[1-6]|blockquote|div|title|style|pre|script|td|li)>\s*/g, '</$2>\n');
						h = h.replace(/\n\n/g, '\n');
					}
				}
...
(function() {
	...
	// This is the XHTML 2.0 transitional elements with it's children packed to reduce it's size
	...
	tinymce.dom.RangeUtils = function(dom) {
		...
		/*		this.split = function(rng) {
			...
			if (startContainer == endContainer) {
				...
				if (startContainer.nodeType == 2 && startOffset != 0) {
					startContainer = splitText(startContainer, startOffset);
					startOffset = 0;
				}

				// Split endContainer text node if needed
				if (endContainer.nodeType == 2 && endOffset != endContainer.nodeValue.length) {
	...
	tinymce.create('tinymce.Editor', {
			...
			function loadScripts() {
				...
				each(explode(s.plugins), function(p) {
					if (p && p.charAt(0) != '-' && !PluginManager.urls[p]) {
						// Skip safari plugin, since it is removed as of 2.3b1
						if (p == 'safari')
							return;
			...
			if (s.custom_elements) {
				function handleCustom(ed, o) {
					each(explode(s.custom_elements), function(v) {
						...
						o.content = o.content.replace(new RegExp('<(' + v + ')([^>]*)>', 'g'), '<' + n + ' _mce_name="$2"$2>');
		...
		setContent : function(h, o) {
			...
			if (!tinymce.isIE && (h.length === 0 || /^\s+$/.test(h))) {
				o.content = t.dom.setHTML(t.getBody(), '<br _mce_bogus="2" />');
				o.format = 'raw';
			}
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			FontSize : function(command, ui, value) {
				...
				// Convert font size 2-7 to styles
				if (value >= 1 && value <= 7) {
	...
	function splitList(selection, dom, li) {
		...
		if (isEmpty(li)) {
			...
			if (!dom.getParent(listBlock.parentNode, 'ul,ol')) {
				...
				block = dom.create('p', 0, '<br _mce_bogus="2" />');
	...
	tinymce.create('tinymce.ForceBlocks', {
		ForceBlocks : function(ed) {
			...
			function padd(ed, o) {
				...
				o.content = o.content.replace(t.rePadd, '<' + elm + '$2$2$2$4$1$6>\u00a0</' + elm + '>');

				if (!isIE && !isOpera && o.set) {
					// Use &nbsp; instead of BR in padded paragraphs
					o.content = o.content.replace(t.reNbsp2BR1, '<' + elm + '$2$2><br /></' + elm + '>');
					o.content = o.content.replace(t.reNbsp2BR2, '<' + elm + '$2$2><br /></' + elm + '>');
				} else
					o.content = o.content.replace(t.reBR2Nbsp, '<' + elm + '$2$2>\u00a0</' + elm + '>');
			};
		...
		setup : function() {
			...
			if (tinymce.isWebKit) {
				function insertBr(ed) {
					...
					if (divYPos > vpHeight) // It is not necessary to scroll if the DIV is inside the views port.
...
//				return TRUE;
			...
			// Move cursor and scroll into views
			r = d.createRange();
			...
			// scrollIntoView seems to scroll the parent window in most browsers now including FF 2.0b4 so it's time to stop using it and do it our selfs
		...
		function mergeSiblings(prev, next) {
			...
			function compareElements(node1, node2) {
				...
				function compareObjects(obj1, obj2) {
					...
					// Check if obj 2 has something obj 2 doesn't have
					for (name in obj2) {
